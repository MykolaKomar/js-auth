...
// post відправляти інформацію про реєстрацію
router.post('/signup', function (req, res) {
  // витягнули з даних через деструктиризацію
  // email, password, role
  const { email, password, role } = req.body
  // вивели їх в консоль лог
...
router.post('/signup-confirm', function (req, res) {
  ...
  if (!code || !token) {
    return res.status(400).json({
      message: "Помилка. Обов'зкові поля відсутні",
    })
  }
...
router.post('/login', function (req, res) {
  ...
  try {
    ...
    if (!user) {
      return res.status(400).json({
        message:
          'Помилка. Користувача з таким email неіснує',
      })
    }
